...
import 'dart:convert';
import '../main.dart'; // Importa variables globales
...
class _MainScreenState extends State<MainScreen> {
  ...
  Widget build(BuildContext context) {
    final List<Widget> screens = [
      TourListScreen(userId: widget.userId, userName: widget.userName),
      ReservasListScreen(userId: widget.userId),
    ];
      ...
      appBar: _currentIndex == 0
          ? AppBar(
              ...
              title: Row(
                ...
                children: [
                  const Icon(Icons.cloud_done, color: Color(0xFF60A5FA)),
      ...
      bottomNavigationBar: Container(
        ...
        child: BottomNavigationBar(
          ...
          items: const [
            ...
            BottomNavigationBarItem(icon: Icon(Icons.receipt_long), label: 'Mis Reservas'),
          ],
        ),
      ),
    );
...
// ==========================================
// PANTALLA 1: CATÁLOGO DE TOURS (CON CUPOS Y TU DISEÑO)
// ==========================================
class TourListScreen extends StatefulWidget {
  final int userId;
  final String userName;
  const TourListScreen({super.key, required this.userId, required this.userName});
...
class _TourListScreenState extends State<TourListScreen> {
  ...
  int _availableSeats(Map tour) {
    final int cupoMaximo = _toInt(tour['cupo_maximo'], fallback: 20);
    final int ocupadas = reservas
        .where((r) => r['tour'] == tour['id'])
        .fold<int>(0, (total, reserva) => total + _toInt(reserva['cantidad_pasajeros']));
    final int restantes = cupoMaximo - ocupadas;
  ...
  Future<void> fetchTours() async {
    ...
    try {
      ...
      final responseReservas = await http.get(Uri.parse(apiUrlReservas));
      if (response.statusCode == 200 && responseReservas.statusCode == 200) {
        setState(() {
          tours = json.decode(utf8.decode(response.bodyBytes));
          reservas = json.decode(utf8.decode(responseReservas.bodyBytes));
          isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _reservarTour(BuildContext context, Map tour) async {
    ...
    int pasajeros = 1;
    int cuposDisponibles = _availableSeats(tour);
    if (context.mounted) {
              ...
              return AlertDialog(
                ...
                title: const Row(
                  children: [
                    ...
                    SizedBox(width: 10),
                    Text('Nueva Reserva', style: TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
                content: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Cupos disponibles: $cuposDisponibles pasajeros',
    ...
    if (pasajeros > cuposDisponibles) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Solo quedan $cuposDisponibles cupos disponibles'), backgroundColor: Colors.red),
        );
      }
      return;
    }
    ...
    double precioUnitario = double.tryParse(tour['precio'].toString()) ?? 0.0;
    ...
    final body = {
      ...
      'precio_total': (precioUnitario * pasajeros).toString()
    };
              ...
              child: tours.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        int cupos = tour['cupo_maximo'] ?? 0;
                        bool agotado = cupos <= 0;
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('Cupos máximos: $cupos', style: TextStyle(color: agotado ? Colors.red : Colors.green, fontWeight: FontWeight.bold)),
                                const SizedBox(height: 20),
                                SizedBox(
                                  width: double.infinity,
                                  child: ElevatedButton.icon(
                                    onPressed: agotado ? null : () => _reservarTour(context, tour),
                                    icon: const Icon(Icons.bookmark_add, color: Colors.white, size: 18),
                                    label: Text(agotado ? 'Agotado' : 'Agendar Reserva', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: agotado ? Colors.grey : const Color(0xFF059669), 
...
// ==========================================
// PANTALLA 2: MIS RESERVAS (CON TUS MODALES)
// ==========================================
class ReservasListScreen extends StatefulWidget {
  final int userId;
  const ReservasListScreen({super.key, required this.userId});
...
class _ReservasListScreenState extends State<ReservasListScreen> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      if (resReservas.statusCode == 200 && resTours.statusCode == 200) {
        List todasLasReservas = json.decode(utf8.decode(resReservas.bodyBytes));
        setState(() {
          // FILTRO PRIVADO: Solo vemos las del usuario actual
          this.todasLasReservas = todasLasReservas;
          reservas = todasLasReservas.where((r) => r['usuario'] == widget.userId).toList();
  ...
  String getTourName(int tourId) {
    final tour = tours.firstWhere((t) => t['id'] == tourId, orElse: () => null);
    return tour != null ? tour['nombre'] : 'Tour Desconocido';
  }
  ...
  int _remainingSeatsForReservation(Map reserva) {
    final tourRelacionado = tours.firstWhere(
      (t) => t['id'] == reserva['tour'],
      orElse: () => null,
    );
    final int cupoMaximo = tourRelacionado != null ? _toInt(tourRelacionado['cupo_maximo'], fallback: 20) : 20;
    final int ocupadasPorOtrasReservas = todasLasReservas
        .where((r) => r['tour'] == reserva['tour'] && r['id'] != reserva['id'])
        .fold<int>(0, (total, reservaActual) => total + _toInt(reservaActual['cantidad_pasajeros']));
    final int restantes = cupoMaximo - ocupadasPorOtrasReservas;
    return restantes < 0 ? 0 : restantes;
  }
  ...
  Future<void> _cancelarReserva(int reservaId) async {
    ...
    try {
      ...
      if (response.statusCode == 204) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reserva cancelada con éxito')));
        fetchData(); 
      } else {
  ...
  Future<void> _editarReserva(Map reserva) async {
    ...
    final int cuposDisponibles = _remainingSeatsForReservation(reserva);
    ...
    if (result != null) {
      ...
      final tourRelacionado = tours.firstWhere((t) => t['id'] == reserva['tour'], orElse: () => null);
      double precioUnitario = tourRelacionado != null ? (double.tryParse(tourRelacionado['precio'].toString()) ?? 0.0) : 0.0;
      ...
      final body = {
        ...
        'fecha': result['fecha'],
        'tour': reserva['tour'],
        'precio_total': nuevoPrecioTotal.toString(),
      };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Control de Reservas')),
      body: isLoading
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF2563EB)))
          : RefreshIndicator(
              onRefresh: fetchData,
                  ...
                  ? ListView(
                      ...
                      children: [
                        Container(
                          height: MediaQuery.of(context).size.height * 0.7,
                          alignment: Alignment.center,
                          child: const Text('No tienes reservas activas.', style: TextStyle(color: Color(0xFF64748B), fontSize: 16)),
                        )
                      ],
                    )
                  : ListView.builder(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Text('\$${res['precio_total']}', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: Color(0xFF059669))),
